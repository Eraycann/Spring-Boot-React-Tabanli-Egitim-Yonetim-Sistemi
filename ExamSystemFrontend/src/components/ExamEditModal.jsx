...
const ExamEditModal = ({ open, onClose, exam, onSuccess }) => {
  ...
  useEffect(() => {
    if (exam) {
      setFormData({
        name: exam.name || '',
        durationInMinutes: exam.durationInMinutes || 45
      });
    }
  }, [exam]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiService.put(`/exams/${exam.id}`, examData);
      
      onSuccess(response);
      handleClose();
    } catch (err) {
      setError(err.message || 'Sınav güncellenirken bir hata oluştu');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
          ...
          <Box className="space-y-4">
            ...
            <Box className="bg-gray-50 p-4 rounded-xl border border-gray-200">
              ...
              <Box className="space-y-2">
                ...
                <Typography variant="body1" className="text-gray-800">
                  <strong>Kurs:</strong> {exam.courseName}
                </Typography>
              </Box>
            </Box>
          </Box>
        ...
        <DialogActions sx={{ p: 3, pt: 1 }}>
          ...
          >
            {loading ? 'Güncelleniyor...' : 'Sınavı Güncelle'}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
